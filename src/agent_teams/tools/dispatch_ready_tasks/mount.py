    ...
    def dispatch_ready_tasks(ctx, workflow_id: str, max_dispatch: int = 4) -> str:
        ...
        def _action() -> str:
            ...
            def _ensure_and_execute(task_id: str, task_name: str, role_id: str) -> bool:
                ...
                dispatched.append({'task_id': task_id, 'task_name': task_name, 'role_id': role_id, 'instance_id': instance.instance_id})
                try:
                    ...
                    failed.append({'task_id': task_id, 'task_name': task_name, 'status': 'failed', 'error': str(exc)})
            ...
            completed_tasks = [
                ...
                if records.get(tasks[name].get('task_id', '')) and records[tasks[name].get('task_id', '')].status == TaskStatus.COMPLETED
            ]
            ...
            return json.dumps(
                {
                    ...
                    'next_action': _next_action(converged_stage, failed, completed_count, total_tasks),
...
def _next_action(converged_stage: str, failed: list[dict[str, str]], completed: int, total: int) -> str:
    if failed:
